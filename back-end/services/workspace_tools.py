...
# 消费 ``delete_granted`` 的工具名。审批放行之后要重建令牌，而那处判定问的是
# "这次被批准的调用本身是不是一次删除"——按名字判，名字就不该散着写。
#
# 少一个名字的后果不是报错，是**用户点了同意、工具却回"需要你明确要求过删除"**：
# 审批闸门与确认令牌的判据不一样（前者问"点没点"，后者问"原话里要求过没有"），
# 而恢复轮里没有新的用户输入，令牌只能由裁决重建。漏掉的那个工具于是永远拿不到
# 令牌，而 run 落 ``done``、工具状态是 ``ok``，日志里什么都看不出来。
#
# 两个名字分属两个模块（知识库文档在本模块，本机文件在 ``fs_tools``）。这里写
# 字面量而不 import fs_tools，与 ``approval._DEFAULT_GATED`` 同一个理由：那边持有
# 数据库会话，而这个元组要在工具执行的边界上被读到。一致性由 test_fs_tools 里
# 一条断言钉住。
DELETE_TOOLS = ("delete_knowledge_document", "delete_file")


# 判定"用户明确要求删除"的意图词。用词表而不是让模型判：模型可能把别处夹带的
# 指令当成用户意图，词表只认用户原话里的字面表达。
#
# 2026-09-05 补 ``清空|干掉``：这个令牌现在同时管着删知识库文档和**删本机文件**
# （fs_tools.delete_file），而说到本机文件时"把它清空""把它干掉"和"删掉"一样常用。
# 不收英文 ``remove``：讨论代码时"把这段 remove 掉"会假阳，而假阳的方向不安全。
#
# 假阴（词表没认出来）的后果只是工具拒绝执行并让模型向用户确认一次，
# 那是安全的方向；假阳才是要避免的。
_DELETE_INTENT = re.compile(r"(删除|删掉|删了|移除|清理|清空|去掉|干掉|不要了)")
# 否定词紧贴在意图词之前时不生效："不要删除""别删"都不算授权。
_DELETE_NEGATED = re.compile(
    r"(不|别|勿|不要|别要)\s?(删除|删掉|删了|移除|清理|清空|去掉|干掉)"
)


def detect_delete_intent(text: str) -> bool:
    """扫描用户原话，判定是否明确要求删除（知识库文档或本机文件）。"""
