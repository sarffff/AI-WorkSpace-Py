...
class HybridRetriever:
    ...
    async def _rerank_via_llm(
        ...
        # 要一份完整排列,不邀请模型省略。原来的提示词写的是"完全不相关的片段
        # 直接省略",但调用方(见 _rerank 末尾)会把漏掉的候选按原相对顺序全部
        # 追加回来——省略一个是空操作,省略全部则是 [] 被契约判成 invalid,
        # 白付一次重试再降级,而降级和全部省略最终都落到同一个融合序。
        #
        # 还有一层:mode=api 的 cross-encoder 给每个候选都打分,从不省略。
        # 邀请 llm 这一支去筛除,会让两个变体干的不是同一件事,而"专用重排比
        # 通用模型好多少"正是这组对照要量的。
        prompt = (
            "下面是候选参考片段。请按与问题的相关程度从高到低排序，"
            f"把全部 {len(passages)} 个编号都排进去，不要省略、不要重复。"
            "只输出片段编号组成的 JSON 数组，"
            "例如 [3, 1, 5]，不要任何解释。\n\n"
            f"问题：{query}\n\n" + "\n\n".join(passages)
        )
